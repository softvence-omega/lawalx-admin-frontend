    ...
    >
      ...
      >
        <Link to="/" className="no-underline">
          ...
          {isCollapsed && (
             <img src="/sitelogo.png" alt="Logo" className="max-h-12 lg:hidden" />
          )}
        </Link>
        {/* Mobile Close Button */}
        <button 
      ...
      >
        {Object.entries(groupedMenu).map(([group, items], index, arr) => (
          <div key={group}>
            ...
            {/* Divider */}
            {index !== arr.length - 1 && (
              <div className={`my-6 border-t border-gray-200 ${isCollapsed ? "lg:mx-0 mx-2" : "mx-2"}`} />
            )}
          </div>
        ))}
      </nav>
      ...
      >
        <UserProfile isCollapsed={isCollapsed && typeof window !== 'undefined' && window.innerWidth >= 1024} />
      </div>
    </aside>
  );
};
